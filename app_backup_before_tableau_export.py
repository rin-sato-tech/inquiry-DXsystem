...
def show_update_form(df: pd.DataFrame) -> None:
    ...
    with st.form("update_inquiry_form"):
        ...
        with col3:
            set_completed_date = st.checkbox(
                "完了日を設定する",
                value=completed_date_enabled_default,
            )

            completed_date_value = st.date_input(
                "完了日",
                value=current_completed_date or date.today(),
                disabled=not set_completed_date,
            )
    ...
    if submitted:
        ...
        assignee = "" if assignee_label == "未設定" else assignee_label
        completed_date_text = completed_date_value.strftime("%Y-%m-%d") if set_completed_date else ""
        ...
        try:
            ...
            st.info("一覧画面に戻ると、更新後の内容を確認できます。")
        except Exception as exc:
            st.error("更新に失敗しました。")
            st.exception(exc)
