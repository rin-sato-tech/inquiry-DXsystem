...
## `additional_info` の保存形式

`additional_info` は、カテゴリ別入力フォームで入力された値を複数行テキストとして保存する。
...
Ver.2では、カテゴリ別の追加情報を個別カラムや別テーブルには分けず、1つのテキストカラムに保存する。 これは、実装をシンプルに保ちつつ、管理部が確認すべき追加情報を一覧・詳細画面で確認できるようにするためである。
